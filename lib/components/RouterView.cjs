...
const RouterView = vue.defineComponent({
  ...
  setup(props, {
    ...
    const parentDepth = vue.inject(ROUTER_VIEW_DEPTH_SYMBOL, 0);
    const currentDepth = parentDepth + 1;
    vue.provide(ROUTER_VIEW_DEPTH_SYMBOL, currentDepth);
    if (!router || !route) {
    ...
    const matchedRecord = vue.computed(() => {
      const matched = route.value?.matched;
      if (!matched?.length) return null;
      return matched[currentDepth - 1];
    });
    const componentDef = vue.computed(() => {
      return matchedRecord.value?.components?.[props.name];
    });
    vue.watch(componentDef, async (newComponentDef, oldComponentDef) => {
      if (!newComponentDef) {
      ...
      try {
        ...
        if (loadedComponent) {
          currentComponent.value = vue.markRaw(loadedComponent);
          error.value = null;
        }
        isLoading.value = false;
      } catch (err) {
    ...
    return () => {
      ...
      if (isLoading.value && props.loading) {
        return slots.loading?.() || vue.h("div", {
      ...
      const wrapWithTransition = (vnode) => {
        ...
        return vue.h(vue.Transition, transitionProps, () => vnode);
      };
      const wrapWithKeepAlive = (vnode) => {
        ...
        return vue.h(vue.KeepAlive, keepAliveProps, () => vnode);
      };
      if (slots.default) {
...
//# sourceMappingURL=RouterView.cjs.map
