...
class I18nRouteManager {
  ...
  applyLocalizedRoutes(locale) {
    ...
    logger.logger.info(`Applied routes for locale: ${locale}`);
  }
  /**
   * 设置导航守卫
   */
  ...
  setLocale(locale) {
    if (!this.config.locales.includes(locale)) {
      logger.logger.warn(`Locale "${locale}" is not supported`);
      return;
    }
    ...
    logger.logger.info(`Locale changed from ${oldLocale} to ${locale}`);
  }
  /**
   * 移除路由中的语言信息
   */
...
function useI18nRoute() {
  ...
  const currentLocale = vue.computed(() => i18nManager.getLocale());
  return {
    // 当前语言
...
exports.I18nRouteManager = I18nRouteManager;
exports.I18nRouterPlugin = I18nRouterPlugin;
exports.getI18nManager = getI18nManager;
exports.setupI18nRouter = setupI18nRouter;
exports.useI18nRoute = useI18nRoute;
