...
var lruCache = require('./matcher/lru-cache.cjs');
...
class RouteMatcher {
  constructor(cacheSize = 100) {
    ...
    this.lruCache = new lruCache.LRUCache(cacheSize);
  ...
  addSegmentToNode(node, segment) {
    if (segment.startsWith(":")) {
      if (!node.paramChild) {
        ...
        node.paramChild.paramName = segment.slice(1).replace(constants.OPTIONAL_PARAM_RE, "");
        node.paramChild.isOptional = constants.OPTIONAL_PARAM_RE.test(segment);
      }
  ...
  buildPathFromParams(pattern, params) {
    return pattern.replace(constants.PARAM_RE, (_match, paramName, optional) => {
...
//# sourceMappingURL=matcher.cjs.map
