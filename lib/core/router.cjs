...
class RouterImpl {
  constructor(options) {
    ...
    this.matcher = new matcher.RouteMatcher();
    this.currentRoute = vue.ref(constants.START_LOCATION);
    this.memoryManager = new unifiedMemoryManager.UnifiedMemoryManager({
      monitoring: {
        enabled: true,
        interval: 6e4,
        // 每分钟检查
  ...
  install(app) {
    app.provide(constants.ROUTER_INJECTION_SYMBOL, this);
    app.provide(constants.ROUTE_INJECTION_SYMBOL, this.currentRoute);
    app.config.globalProperties.$router = this;
    app.config.globalProperties.$route = this.currentRoute;
    app.component("RouterLink", RouterLink.RouterLink);
    app.component("RouterView", RouterView.RouterView);
  }
  /**
   * 销毁路由器，清理所有资源
   */
  ...
  async pushWithRedirect(to, replace) {
    ...
    if (this.redirectCount >= this.MAX_REDIRECTS) {
      ...
      return this.createNavigationFailure(constants.NavigationFailureType.aborted, this.currentRoute.value, this.resolve(to));
    }
    const targetLocation = this.resolve(to);
    const from = this.currentRoute.value;
    if (this.isSameRouteLocation(targetLocation, from)) {
      this.redirectCount = 0;
      return this.createNavigationFailure(constants.NavigationFailureType.duplicated, from, targetLocation);
    }
    try {
      ...
      if (error instanceof Error) {
        this.handleError(error);
        this.redirectCount = 0;
        return this.createNavigationFailure(constants.NavigationFailureType.aborted, from, targetLocation);
      }
      this.redirectCount = 0;
      throw error;
    }
