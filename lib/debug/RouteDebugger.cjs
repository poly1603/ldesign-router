...
class RouteTracer {
  ...
  log(level, message, data) {
    ...
    if (messageLevel >= configLevel) {
      ...
      logger.logger[method](`[Router] ${message}`, data || "");
    }
  }
  // 获取历史记录
  getHistory() {
...
class RoutePerformanceAnalyzer {
      ...
      if (metrics && metrics.length > 0) {
        ...
        if (lastMetric && !lastMetric.endTime) {
          ...
          if (lastMetric.duration && lastMetric.duration > (this.config?.slowThreshold || 100)) {
            logger.logger.warn(`\u26A0\uFE0F Slow route detected: ${to.path} took ${lastMetric.duration.toFixed(2)}ms`);
  ...
  analyzeSlow(_route, metric) {
    ...
    if (suggestions.length > 0) {
      logger.logger.info("\u{1F4CA} Performance suggestions:");
      suggestions.forEach((s) => logger.logger.info(`  - ${s}`));
    }
  }
  // 开始自动报告
  startAutoReporting() {
  ...
  generateReport() {
    ...
    logger.logger.info("\u{1F4CA} Performance report generated", report);
    return report;
  }
  // 计算百分位数
  percentile(values, p) {
...
class RouteErrorDiagnostics {
  ...
  outputDiagnosis(record) {
    ...
    logger.logger.group(`\u{1F50D} Error Diagnosis [${diagnosis.severity}]`);
    logger.logger.error("Error:", record.error.message);
    logger.logger.info(`Category: ${diagnosis.category}`);
    logger.logger.info(`Type: ${diagnosis.type}`);
    logger.logger.info(`Route: ${record.route}`);
    if (diagnosis.suggestions.length > 0) {
      logger.logger.info("Suggestions:");
      diagnosis.suggestions.forEach((s) => logger.logger.info(`  - ${s}`));
    }
    if (record.error.stack) {
      logger.logger.debug("Stack trace:", record.error.stack);
    }
  ...
  async reportError(record) {
    ...
    try {
      ...
      logger.logger.error("Failed to report error:", e);
    }
  }
  // 生成ID
  generateId() {
...
class RouteDebugger {
  ...
  setupConsoleCommands() {
    if (typeof window !== "undefined") {
      ...
      logger.logger.info("[RouteDebugger] Console commands registered. Use window.routeDebugger to access.");
    }
  }
  // 获取调试信息
  getDebugInfo() {
...
function debugLog(...args) {
  logger.logger.info("[RouteDebug]", ...args);
}
function debugWarn(...args) {
  logger.logger.warn("[RouteDebug Warning]", ...args);
}
function debugError(...args) {
  logger.logger.error("[RouteDebug Error]", ...args);
}

exports.RouteDebugger = RouteDebugger;
exports.RouteErrorDiagnostics = RouteErrorDiagnostics;
exports.RoutePerformanceAnalyzer = RoutePerformanceAnalyzer;
exports.RouteTracer = RouteTracer;
exports.RouteVisualizer = RouteVisualizer;
exports.debugError = debugError;
exports.debugLog = debugLog;
exports.debugWarn = debugWarn;
exports.getDebugInfo = getDebugInfo;
exports.getRouteDebugger = getRouteDebugger;
exports.setupRouteDebugger = setupRouteDebugger;
/*! End of @ldesign/router | Powered by @ldesign/builder */
//# sourceMappingURL=RouteDebugger.cjs.map
