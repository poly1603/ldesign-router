...
class ErrorManager {
  ...
  setupGlobalErrorHandlers() {
    if (typeof window !== "undefined") {
      window.addEventListener("error", (event) => {
        this.handleError({
          type: exports.ErrorType.UNKNOWN,
  ...
  getDefaultRecoveryStrategies() {
    ...
    strategies.set(exports.ErrorType.NAVIGATION, {
      shouldRecover: (error) => error.retryCount < 3,
      recover: async (error) => {
        logger.logger.info("Attempting to recover from navigation error", error);
      ...
      fallback: () => {
        logger.logger.warn("Navigation recovery failed, redirecting to home");
        if (typeof window !== "undefined") {
          window.location.href = "/";
        }
      }
    });
    strategies.set(exports.ErrorType.NETWORK, {
      shouldRecover: (error) => error.retryable !== false && (error.retryCount || 0) < 3,
      recover: async (error) => {
        logger.logger.info("Attempting to recover from network error", error);
        await new Promise((resolve) => setTimeout(resolve, 1e3 * (error.retryCount || 1)));
      }
    });
    strategies.set(exports.ErrorType.COMPONENT, {
      shouldRecover: () => true,
      recover: (error) => {
        logger.logger.info("Component error detected, attempting recovery", error);
      }
    });
    return strategies;
  }
  ...
  logError(error) {
    ...
    switch (error.severity) {
      ...
      case exports.ErrorSeverity.HIGH:
        logger.logger.error(logMessage, error);
        break;
      case exports.ErrorSeverity.MEDIUM:
        logger.logger.warn(logMessage, error);
        break;
      case exports.ErrorSeverity.LOW:
        logger.logger.info(logMessage, error);
        break;
    }
  }
  /**
  ...
  notifyListeners(error) {
    this.listeners.forEach((listener) => {
      try {
        ...
        logger.logger.error("Error in error listener", e);
      }
    });
  }
  /**
  ...
  async attemptRecovery(error) {
    ...
    try {
      ...
      logger.logger.info("Successfully recovered from error", error);
    } catch (recoveryError) {
      logger.logger.error("Recovery failed", recoveryError);
      if (strategy.fallback) {
        strategy.fallback();
   ...
   * 包装函数以捕获错误
   */
  static wrapFunction(fn, errorType = exports.ErrorType.UNKNOWN) {
    return (...args) => {
      try {
   ...
   * 装饰器：错误处理
   */
  static errorHandler(errorType = exports.ErrorType.UNKNOWN) {
