/**
 * Svelte Router Engine 插件
 *
 * 将 Svelte Router 功能集成到 LDesign Engine 中
 */
 ...
 * @example
 * ```typescript
 * import { createRouterEnginePlugin } from '@ldesign/router-svelte'
 *
...
export function createRouterEnginePlugin(
  ...
  if (debug) {
    console.log('[Svelte Router Plugin] createRouterEnginePlugin called with options:', options)
  }
  ...
  return {
    ...
    async install(context: any) {
      try {
        if (debug) {
          console.log('[Svelte Router Plugin] install method called')
        }
        ...
        // 记录安装信息
        engine.logger?.info?.('Installing Svelte router plugin...', {
          version,
          mode,
          base,
        ...
        engine.logger?.info?.('Svelte router plugin installed successfully')
      } catch (error) {
        const engine: EngineLike = context?.engine || context
        engine?.logger?.error?.('Failed to install Svelte router plugin:', error)
        throw error
      }
    },
    ...
    async uninstall(context: any) {
      try {
        ...
        engine.logger?.info?.('Uninstalling Svelte router plugin...')
        ...
        engine.logger?.info?.('Svelte router plugin uninstalled successfully')
      } catch (error) {
        const engine: EngineLike = context?.engine || context
        engine?.logger?.error?.('Failed to uninstall Svelte router plugin:', error)
      }
    },
  }
}
