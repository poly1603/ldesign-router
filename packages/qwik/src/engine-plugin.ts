/**
 * Qwik Router Engine 插件
 *
 * 将 Qwik Router 功能集成到 LDesign Engine 中
 */
 ...
 * @example
 * ```typescript
 * import { createRouterEnginePlugin } from '@ldesign/router-qwik'
 *
...
export function createRouterEnginePlugin(
  ...
  if (debug) {
    console.log('[Qwik Router Plugin] createRouterEnginePlugin called with options:', options)
  }
  ...
  return {
    ...
    async install(context: any) {
      try {
        if (debug) {
          console.log('[Qwik Router Plugin] install method called')
        }
        ...
        // 记录安装信息
        engine.logger?.info?.('Installing Qwik router plugin...', {
          version,
          mode,
          base,
        ...
        engine.logger?.info?.('Qwik router plugin installed successfully')
      } catch (error) {
        const engine: EngineLike = context?.engine || context
        engine?.logger?.error?.('Failed to install Qwik router plugin:', error)
        throw error
      }
    },
    ...
    async uninstall(context: any) {
      try {
        ...
        engine.logger?.info?.('Uninstalling Qwik router plugin...')
        ...
        engine.logger?.info?.('Qwik router plugin uninstalled successfully')
      } catch (error) {
        const engine: EngineLike = context?.engine || context
        engine?.logger?.error?.('Failed to uninstall Qwik router plugin:', error)
      }
    },
  }
}
