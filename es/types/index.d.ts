...
export type ExtractRouteParams<T extends string> = T extends `${infer _Start}:${infer Param}/${infer Rest}` ? {
    ...
    [K in Param]?: string;
} & ExtractRouteParams<Rest> : T extends `${infer _Start}:${infer Param}` ? {
    [K in Param]: string;
 ...
 * 类型安全的路由参数
 */
export type TypedRouteParams<T extends Record<string, any> = Record<string, any>> = {
    [K in keyof T]: T[K] extends string ? string : string | string[];
};
/**
 * 类型安全的查询参数
 */
export type TypedRouteQuery<T extends Record<string, any> = Record<string, any>> = {
    [K in keyof T]?: T[K] extends string ? string | string[] | null | undefined : T[K] extends number ? string | string[] | null | undefined : T[K] extends boolean ? string | string[] | null | undefined : string | string[] | null | undefined;
};
/**
 * 路由元信息类型
 */
 ...
 * 路由位置基础接口（泛型版）
 */
export interface RouteLocationBase<TParams extends Record<string, any> = Record<string, any>, TQuery extends Record<string, any> = Record<string, any>, TMeta extends RouteMeta = RouteMeta> {
    /** 路径 */
    path: string;
    /** 路由名称 */
    name?: string | symbol;
 ...
 * 标准化的路由位置（泛型版）
 */
export interface RouteLocationNormalized<TParams extends Record<string, any> = Record<string, any>, TQuery extends Record<string, any> = Record<string, any>, TMeta extends RouteMeta = RouteMeta> extends RouteLocationBase<TParams, TQuery, TMeta> {
    /** 完整路径（包含查询参数和哈希） */
    fullPath: string;
    /** 匹配的路由记录 */
 ...
 * 类型安全的路由位置（基于路径推导）
 */
export type TypedRouteLocation<T extends string> = RouteLocationNormalized<RouteParamsFor<T>, Record<string, any>, RouteMeta>;
/**
 * 路由位置原始类型（用于导航）
 */
export type RouteLocationRaw<TParams extends Record<string, any> = Record<string, any>, TQuery extends Record<string, any> = Record<string, any>> = string | RouteLocationPathRaw<TQuery> | RouteLocationNamedRaw<TParams, TQuery>;
/**
 * 基于路径的路由位置（泛型版）
 */
export interface RouteLocationPathRaw<TQuery extends Record<string, any> = Record<string, any>> {
    path: string;
    query?: TypedRouteQuery<TQuery>;
    hash?: string;
    state?: HistoryState;
 ...
 * 基于名称的路由位置（泛型版）
 */
export interface RouteLocationNamedRaw<TParams extends Record<string, any> = Record<string, any>, TQuery extends Record<string, any> = Record<string, any>> {
    name: string | symbol;
    params?: TypedRouteParams<TParams>;
    query?: TypedRouteQuery<TQuery>;
    hash?: string;
...
export interface NavigationHookAfter {
    (to: RouteLocationNormalized, from: RouteLocationNormalized, failure?: NavigationFailure | void | undefined): void;
}
/**
 * 历史状态
 */
...
export interface ScrollBehavior {
    (to: RouteLocationNormalized, from: RouteLocationNormalized, savedPosition: ScrollPosition | null): ScrollPosition | Promise<ScrollPosition> | undefined | null;
}
/**
 * 路由器选项
 */
...
export interface DeviceGuardOptions {
    /** 支持的设备类型检查函数 */
    checkSupportedDevices?: (supportedDevices: DeviceType[], currentDevice: DeviceType, route: RouteLocationNormalized) => boolean;
    /** 不支持设备时的处理函数 */
    onUnsupportedDevice?: (currentDevice: DeviceType, route: RouteLocationNormalized) => RouteLocationRaw | void;
}
/**
 * 设备路由插件选项
 */
