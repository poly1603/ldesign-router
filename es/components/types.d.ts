...
import type { ExtractRouteParams, RouteLocationNormalized, RouteLocationRaw } from '../types';
/**
 * 组件尺寸
 */
 ...
 * 链接变体
 */
export type LinkVariant = 'default' | 'primary' | 'secondary' | 'success' | 'warning' | 'danger';
/**
 * 预加载策略
 */
 ...
 * RouterLink 属性（泛型版）
 */
export interface RouterLinkProps<TParams extends Record<string, unknown> = Record<string, unknown>, TQuery extends Record<string, unknown> = Record<string, unknown>> {
    /** 目标路由 */
 ...
 */
export type TypedRouterLinkProps<T extends string> = RouterLinkProps<ExtractRouteParams<T>, Record<string, unknown>> & {
    to: T | RouteLocationRaw<ExtractRouteParams<T>>;
};
/**
 * RouterLink 插槽属性（泛型版）
 */
export interface RouterLinkSlotProps<TParams extends Record<string, unknown> = Record<string, unknown>, TQuery extends Record<string, unknown> = Record<string, unknown>> {
    /** 链接地址 */
    href: string;
    /** 目标路由 */
 ...
 * RouterView 属性（泛型版）
 */
export interface RouterViewProps<TParams extends Record<string, unknown> = Record<string, unknown>, TQuery extends Record<string, unknown> = Record<string, unknown>> {
    /** 视图名称 */
    name?: string;
    /** 路由位置 */
    route?: RouteLocationNormalized<TParams, TQuery>;
...
export interface PermissionChecker {
    (permission: string | string[], route: RouteLocationNormalized): boolean | Promise<boolean>;
}
/**
 * 权限配置
 */
